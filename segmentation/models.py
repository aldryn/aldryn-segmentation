...
class SegmentLimitPluginModel(CMSPlugin):
    ...
    def __unicode__(self):
        if self.label:
            return _(u'%s [%s]') % (self.label, self.configuration_string, )
        else:
...
class SegmentBasePluginModel(CMSPlugin):
    ...
    def configuration_string(self):
        '''
        Return a ugettext_lazy object that represents the configuration for the
...
class SwitchSegmentPluginModel(SegmentBasePluginModel):
    ...
    def configuration_string(self):
        if self.on_off:
            return _noop(u'Always ON')
        else:
            return _noop(u'Always OFF')
...
class CookieSegmentPluginModel(SegmentBasePluginModel):
    ...
    @property
    def configuration_string(self):
        return _noop(u'“%(key)s” equals “%(value)s”') % {'key': self.cookie_key, 'value': self.cookie_value, }
...
class CountrySegmentPluginModel(SegmentBasePluginModel):
    ...
    #
    COUNTRY_CODES = [ (code, _(u'%s: %s') % (code, name)) for (code, name) in COUNTRY_CODES ]
    ...
    @property
    def configuration_string(self):
        return _noop(u'%s (%s)') % (self.country_code_names[self.country_code], self.country_code)
...
class AuthenticatedSegmentPluginModel(SegmentBasePluginModel):
    ...
    @property
    def configuration_string(self):
        return _noop(u'is Authenticated')
